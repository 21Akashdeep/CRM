class Grn {
    ...
    static init() {
        ...
        $('#Grn-BtnSave').on('click', () => {
            ...
            let GrnItem = $('#tableGrn').bootstrapTable('getData').filter(x => x.ItemId > 0);
            //if (GrnItem.length == 0) {
    ...
    static getViewOption() {
        Data.get({
            ...
            onSuccess: (response) => {
                ...
                Dropdown.bind({ id: '#ListId', data: response.data.Grn, value: 'Id', text: 'GrnNo', subText: 'Date' });
    ...
    static get({ method = 'Get', onSuccess }) {
        let obj = {
            ...
            FromDate: $('#DateRange').val().split('|')[0],
            ToDate: $('#DateRange').val().split('|')[1],
            ListId: $('#ListId').val(),
            ListPartyId: $('#ListPartyId').val()
        };
        Data.post({ url: `Grn/${method}`, data: obj, onSuccess: onSuccess });
    }
    static newEntry() {
        Grn.getAddOption({
            onSuccess: (response) => {
                Grn.item = response.data.Item;
                Dropdown.bind({ id: '#Gdn_CustomerId', data: response.data.Customer, value: 'Id', text: 'Description' });
    ...
    static addGrnItem() {
        ...
        Table.add({ id: '#tableGrn', data: obj, action: 'append', selectPick: true });
