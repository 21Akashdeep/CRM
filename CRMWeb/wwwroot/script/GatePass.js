...
class GatePass {
    ...
    static newEntry() {
        GatePass.getAddOption((response) => {
            ...
            Dropdown.bind({ id: '#GatePass_Employee', data: response.data.Employee, value: 'Id', text: 'Name' });
    ...
    static update(obj) { Data.update({ url: 'GatePass/Update', data: obj, onSuccess: GatePass.updateOnSuccess }); }
    ...
    static delete({ id }) {
        Message.confirm({
            msg: 'Do you want to delete??', confirmButtonText: 'Delete', denyButtonText: 'Dont Delete', data: id,
            onConfirm: (id) => { Data.delete({ url: `GatePass/Delete?Id=${id}`, onSuccess: GatePass.deleteOnSuccess }); }
        });
    }
    ...
    static enable({ id }) {
        Message.confirm({
            msg: 'Do you want to enable??', confirmButtonText: 'Enable', denyButtonText: 'Dont Enable', data: id,
            onConfirm: (id) => { Data.update({ url: `GatePass/Enable?Id=${id}`, onSuccess: GatePass.enableOnSuccess }); }
        });
    }
...
window.getExpiryColorStyle = (dateValue) => {
    ...
    let dayGap = expiry.diff(today, 'days');
...
window.tableGatePassName = (value, obj, index) => {
    let name = (obj.EmployeeDesc || "").match(/.{1,30}/g)?.join('<br>') || "-";
    return `<div class="text-cell">${name}</div>`;
}
window.tableGatePassLocation = (value, obj, index) => {
    let loc = (obj.LocationDesc || "").match(/.{1,30}/g)?.join('<br>') || "-";
    return `<div class="text-cell">${loc}</div>`;
}
window.tableGatePassPassType = (value, obj, index) => {
    return `<div class="text-cell">${obj.TypeDesc || "-"}</div>`;
}
window.tableGatePassPassNo = (value, obj, index) => {
    return `<div class="text-cell">${obj.GatePassNo || "-"}</div>`;
}
window.tableGatePassIssueOn = (value, obj, index) => {
    return `<div class="text-cell">${moment(obj.IssuedOn).format('DD-MMM-YYYY')}</div>`;
}
...
window.tableGatePassTrainingExpiryOn = (value, obj, index) => {
    return `<div class="full-fill" style="${window.getExpiryColorStyle(obj.TrainingExpiryOn)}">${moment(obj.TrainingExpiryOn).format('DD-MMM-YYYY')}</div>`;
}
...
window.tableGatePassUpdatedByAndAt = (value, obj, index) => {
    return `<div class="text-cell" style="font-size: 7pt;"><b>${obj.UpdatedByName}</b><br>${moment(obj.UpdatedAt).format('DD-MMM-YYYY HH:mm')}</div>`;
}
