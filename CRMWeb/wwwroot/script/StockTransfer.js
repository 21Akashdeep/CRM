class StockTransfer {
    ...
    static init() {
        ...
        $('#StockTransferScan-ItemSerialNo').on('keydown', (e) => {
            ...
            let item = StockTransfer.itemWithSerialNo.find(x => x.SerialNo == $('#StockTransferScan-ItemSerialNo').val());
            if (!item) {
                Message.error({ statusText: `Invalid Serial No. (${$('#StockTransferScan-ItemSerialNo').val()})` });
                return;
            }
    ...
    static edit({ id, action = 'Edit' }) {
        Data.get({
            ...
            onSuccess: (response) => {
                ...
                StockTransfer.item = option.StockItem;
                let store = option.AddOption.Store;
                let obj = response.data.Voucher;
                //Stocktransfer.item =      
                Dropdown.bind({ id: '#FromStoreId', data: store, value: 'Id', text: 'Description' });
                Dropdown.bind({ id: '#ToStoreId', data: store, value: 'Id', text: 'Description' });
...
window.tableStockTransferItemQty = (value, obj, index) => {
    return `
         <div class="input-group">
            <input type="text" id="StockTransferItemQty_${index}" class="form-control form-control-sm text-end stocktransfer-item-qty" value="${Math.abs(obj.Qty)}" 
...
window.tableStockTransferItemExpiryOn = (value, obj, index) => {
...
window.tableStockTransferScanExpiryOn = (value, obj, index) => {
