    ...
    public class RepoComplaint
    {
        ...
        public async Task<List<Complaint>> ListAsync(Complaint? obj, User User) 
        {
            ...
            if (obj.ListPartyId.Any())
                dbComplaintQuery = dbComplaintQuery.Where(co => obj.ListPartyId.Contains(co.PartyId));            
        ...
        public async Task<Message> UpdateAsync(Complaint obj, User User)
        {
            ...
            try
            {
                ...
                UpdateComplaint.PartyId = obj.PartyId;
        ...
        public async Task<Message> NotifyComplaintLogByEmail(Complaint obj, bool IsReg)
        {
            ...
            try 
            {
                ...
                if (IsReg)
                {
                    string PartyMailBody = $"Your complaint No. {obj.Code} has been registered. ";
                    var PartyMail = new MailMessage();
                    PartyMail.To.Add(obj.Email);
                    Util.SentMail(db, PartyMail, $"Complaint Registered: {obj.Code}", PartyMailBody, "info", ref objMsg);
